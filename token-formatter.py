...
def table(elapsed_ms, b, style):
    ...
    table.add_column(
        "Time Taken", justify="right", header_style="light_green", style="bright_green"
    )
...
def txt_to_json_pw():
    ...
    with open(f"{application_path}\data\{'tokens.json'}") as f:
        t = json.load(f)
    with open(f"{application_path}\data\{'tokens.txt'}") as f:
    ...
    with open(f"{application_path}\data\{'tokens.json'}", "w") as f:
        json.dump(t, f)
    e = time()
    elapsed = e - s
...
def txt_to_txt():
    ...
    with open(f"{application_path}\data\{'tokens.txt'}") as f:
        tkns = [i.strip() for i in f]
    txtfile = input(
        "What's the name of your output file \x1b[38;5;9m(\x1b[0mLeave blank for output.txt\x1b[38;5;9m)\x1b[0m? -> "
    )
    print()
    if len(txtfile) == 0:
        txtfile = "output.txt"
...
logging.info("")
choose = input(
    """\033[92m[\x1b[0m1\033[92m]\x1b[0m token -> token  \x1b[38;5;9m(\x1b[0mtxt -> json\x1b[38;5;9m)\x1b[0m
